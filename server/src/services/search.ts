...
// Primary backend: Brave Search API (https://api.search.brave.com) — reliable,
// no scraping, free tier (2000 queries/month). Requires BRAVE_SEARCH_API_KEY.
...
// directing the user to set BRAVE_SEARCH_API_KEY.
...
export interface SearchResponse {
  ...
  cached: boolean;
  /** Which backend produced these results: "brave" | "duckduckgo". */
  ...
  constructor() {
    super(
      "DuckDuckGo is blocking this server's IP with an anti-bot challenge. " +
        "Set BRAVE_SEARCH_API_KEY (free tier: 2000 queries/month) for reliable web search."
    );
 ...
 * Uses Brave Search API when BRAVE_SEARCH_API_KEY is set (reliable, no scraping).
 * Falls back to DuckDuckGo HTML scraping (no key required, but may be blocked
 * on datacenter IPs). Includes a short in-memory cache to dedupe repeated
 * queries inside a multi-step Athena tool loop.
 */
export async function webSearch(
  ...
  // Prefer Brave (reliable) when configured.
  if (hasBrave) {
    try {
      ...
      // If Brave fails, fall through to DDG if available.
      lastError = e instanceof Error ? e : new Error(String(e));
    }
  }

  // Fallback: DuckDuckGo (no key required, but may be IP-blocked).
  try {
    ...
    // If Brave was configured and failed, prefer its error message (more
    // actionable) unless DDG was the one that was blocked.
    if (lastError && !(ddgErr instanceof DdgBlockedError)) throw lastError;
    throw ddgErr;
  }
}
